...
def exp_events_handle(experiment_env, exp_type, start_key_down, t0, record_key = ''):
    ...
    for event in pygame.event.get():
        if event.type in (QUIT, MOUSEBUTTONDOWN):
            ...
            pygame.quit()
            sys.exit()
        elif (event.type == KEYDOWN and (event.key == K_1 or event.key == K_KP1)):
        ...
        elif (event.type == KEYUP and (event.key == K_1 or event.key == K_KP1)):
            # Write result file's headers
        ...
        elif (event.type == KEYDOWN and (event.key == K_2 or event.key == K_KP2)):
            # Write result file's headers    
            timestamp = pygame.time.get_ticks() - t0
        ...
        elif (event.type == KEYUP and (event.key == K_2 or event.key == K_KP2)):
            # Write result file's headers
        ...
        elif (event.type == KEYDOWN and (event.key == K_3 or event.key == K_KP3)):
            # Write result file's headers    
            timestamp = pygame.time.get_ticks() - t0
        ...
        elif (event.type == KEYUP and (event.key == K_3 or event.key == K_KP3)):
            # Write result file's headers
...
# Ending experiment
def experiment_end(experiment_env, exp_type, initial_id_answer, surf, start_key_down, t0):
    
    # Analyzing pressed keys
    pressed_keys = pygame.key.get_pressed()
    if pressed_keys[K_1] or pressed_keys[K_KP1]:
        exp_events_handle(experiment_env, exp_type, start_key_down, t0, 'K_1')
    if pressed_keys[K_2] or pressed_keys[K_KP2]:
        exp_events_handle(experiment_env, exp_type, start_key_down, t0, 'K_2')
    if pressed_keys[K_3] or pressed_keys[K_KP3]:
        exp_events_handle(experiment_env, exp_type, start_key_down, t0, 'K_3')
